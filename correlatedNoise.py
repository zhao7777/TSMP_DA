...
def copy_attr_dim(src, dst):
    ...
    dst.setncattr("perturbed_by", "T.Boas")
...
#%% Parameters
outputpath = '/p/project/detectc01/clm_inputfiles/forcings/noise/'
...
rho = 7/8 # decorrelation time 24h. time step of forcings 3h --> decorrelation after one days which means 3h are 1/8 --> 7/8 decorrelation time step

# Covariance matrix for fields
C_var = np.array([[1, 0, -0.8, 0.5],
                 [0, 1, 0.4, 0.4],
                 [-0.8, 0.4, 1, -0.5],
                 [0.5, 0.4, -0.5, 1]])
...
rnd_state_file = "/p/project/detectc01/clm_inputfiles/forcings/noise/rnd_state.json"
force_seed = True
...
fname = ("/p/project/detectc01/clm_inputfiles/forcings/era5/2011-01.nc")
...
for year, month in zip(years, months * len(years)):
    ...
    fname = ("/p/project/detectc01/clm_inputfiles/forcings/era5/" + str(year) + "-" + str(month).zfill(2) + ".nc")
    ...
    with nc.Dataset(filename, "w") as ncid:
        ...
        for t in range(int(t_start),int(timeNum)):
            ...
            e_corrSpatTemp_PREC = rho*e_prev_PREC+np.sqrt(1-rho**2)*e_corrSpat_PREC
            e_corrSpatTemp_TBOT = rho*e_prev_TBOT+np.sqrt(1-rho**2)*e_corrSpat_TBOT
            e_corrSpatTemp_FSDS = rho*e_prev_FSDS+np.sqrt(1-rho**2)*e_corrSpat_FSDS
            e_corrSpatTemp_FLDS = rho*e_prev_FLDS+np.sqrt(1-rho**2)*e_corrSpat_FLDS
            del e_corrSpat_PREC, e_corrSpat_TBOT, e_corrSpat_FSDS, e_corrSpat_FLDS
            ...
            #%% Truncate noise
            e_corrSpatTemp_PREC = 1+std[0]*e_corrSpatTemp_PREC
            ...
            e_corrSpatTemp_FSDS = 1+std[2]*e_corrSpatTemp_FSDS
            ...
            del e_corrSpatTemp_PREC, e_corrSpatTemp_TBOT, e_corrSpatTemp_FSDS, e_corrSpatTemp_FLDS
